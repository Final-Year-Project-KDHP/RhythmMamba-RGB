...
# IMPORTANT: Import the mini model instead of the original
from neural_methods.model.RGBMamba_mini import RGBMamba_mini
from neural_methods.trainer.BaseTrainer import BaseTrainer
from neural_methods.loss.TorchLossComputer import Hybrid_Loss


class RGBMamba_miniATrainer(BaseTrainer):
    """
    Trainer for the mini version of RGBMamba. 
    Its logic is nearly the same as the original trainer, 
    but it references the RGBMamba_mini model instead of RGBMamba.
    """
    ...
    def __init__(self, config, data_loader):
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            ...
            self.model = RGBMamba_mini().to(self.device)
            ...
            # Create the mini model for testing only
            self.model = RGBMamba_mini().to(self.device)
            self.model = torch.nn.DataParallel(self.model, device_ids=list(range(self.num_of_gpu)))
        else:
            raise ValueError("RGBMambaTrainer_mini initialized in incorrect toolbox mode!")
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                print(f"[DEBUG Trainer] data.shape after augmentation = {data.shape}")
                # Should be [B, D, 3, H, W]
