...
class BMambaTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            self.model = BMamba().to(self.device)
            ...
            raise ValueError("BMamba trainer initialized in incorrect toolbox mode!")

    def preprocess_blue_channel(self, data):
        """
        Extract the blue channel from the input data.

        Args:
            data: input data tensor of shape (N, D, C, H, W)

        Returns:
            data_g: data tensor containing only the blue channel

        Note:
            The blue channel is at index 1 in the channel dimension.

        """
        data_b = data[:, :, 2:3, :, :]  # index 2 for the B channel
        return data_b
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                # Preprocess: extract blue channel if desired
                data = self.preprocess_blue_channel(data)
    ...
    def valid(self, data_loader):
        ...
        with torch.no_grad():
            ...
            for valid_idx, valid_batch in enumerate(vbar):
                ...
                data_valid = self.preprocess_blue_channel(data_valid)
    ...
    def test(self, data_loader):
        ...
        with torch.no_grad():
            ...
            for _, test_batch in enumerate(tbar):
                ...
                # Optionally preprocess blue channel for test
                data_test = self.preprocess_blue_channel(data_test)
