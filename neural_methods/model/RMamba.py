...
class Fusion_Stem(nn.Module):
    ...
    def forward(self, x):
        ...
        print("Fusion_Stem output shape:", x.shape)
        return x
...
class Block_mamba(nn.Module):
    ...
    def forward(self, x):
        ...
        print("Block_mamba output shape:", x.shape)
        return x
...
class RMamba(nn.Module):
    def __init__(self, depth=24, embed_dim=96, mlp_ratio=2, drop_path_rate=0.1):
        ...
        self.blocks = nn.ModuleList([Block_mamba(embed_dim, mlp_ratio, drop_path=dpr[i]) for i in range(depth)])
