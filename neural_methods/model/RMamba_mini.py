...
"""
Mini version of RMamba:
  - Reduced channel sizes by ~1/3
  - embed_dim = 32 (instead of 96)
  - Fusion_Stem dim= embed_dim//4 = 8 (instead of 24)
  - Mamba d_state=16 (instead of 48), etc.
"""
...
class Fusion_Stem_mini(nn.Module):
    ...
    def forward(self, x):
        ...
        print("Fusion_Stem_mini output shape:", x.shape)
        return x
...
class MambaLayer_mini(nn.Module):
    """
    Mamba with smaller d_state=16 instead of 48, matching 1/3 scaling
    """
...
class Block_mamba_mini(nn.Module):
    def __init__(self, dim, mlp_ratio=2, drop_path=0.):
        ...
        self.attn = MambaLayer_mini(dim, d_state=16)  # scaled down
    ...
    def forward(self, x):
        ...
        print("Block_mamba_mini output shape:", x.shape)
        return x


class RMamba_mini(nn.Module):
    """
    Scaled-down version of RMamba.
     - embed_dim=32 (down from 96)
     - The "Fusion_Stem" uses dim= embed_dim//4 = 8
     - The 3D conv has in_channels= 8, out_channels= 32
     - Everything else is the same structure, but smaller channels
    """
    ...
    def __init__(self, depth=24, embed_dim=32, mlp_ratio=2, drop_path_rate=0.1):
        ...
        # e.g. in_channels = embed_dim // 4 = 8 => out_channels = embed_dim=32
    ...
    def forward(self, x):
        ...
        # new shape => [B, D, 8, H//8, W//8], then permute
        ...
        print("Final Output Shape (normalized):", x.shape)
        return x
