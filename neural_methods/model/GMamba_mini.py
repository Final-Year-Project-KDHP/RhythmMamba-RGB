...
"""
Mini version of GMamba
Same idea: embed_dim=32, Fusion_Stem(dim=8), etc.
"""
...
class Fusion_Stem_mini(nn.Module):
    ...
    def forward(self, x):
        ...
        print("Fusion_Stem_mini (G) output shape:", x.shape)
        return x
...
class Block_mamba_mini(nn.Module):
    ...
    def forward(self, x):
        ...
        print("Block_mamba_mini (G) output shape:", x.shape)
        return x


class GMamba_mini(nn.Module):
    ...
    def forward(self, x):
        ...
        print("After Fusion_Stem (G):", x.shape)

        x = self.stem3(x)
        print("After stem3 (G):", x.shape)
        ...
        print("After Mean Pooling (G):", x.shape)

        x = rearrange(x, 'b c t -> b t c')

        for i, blk in enumerate(self.blocks):
            x = blk(x)
            print(f"After Block {i + 1} (G):", x.shape)
        ...
        print("Final Output Shape (G, normalized):", x.shape)
        return x
